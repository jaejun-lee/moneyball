...
def match_names_fixture_salary_fuzzywuzzy(lst_A, lst_B, dict_names = {}, cutoff=90, scorer=fuzz.WRatio):
    '''
    ...
    '''
    for name_a in lst_A:
        if name_a not in dict_names:
            name_b = process.extractOne(unidecode.unidecode(name_a), lst_B, scorer=scorer, score_cutoff=cutoff)
            if name_b:
                dict_names[name_a] = name_b[0] #name_b tuple(name, score)
    return None
...
if __name__=='__main__':
    ...
    col_salaries = [db.salary_2019, db.salary_2018, db.salary_2017, db.salary_2016, db.salary_2015, db.salary_2014]
    ...
    for col_fixture in col_fixtures:
        ...
        for salary in col_salaries:
            ...
            match_names_fixture_salary_fuzzywuzzy(lst_A_names, lst_B_names, dict_names, scorer=fuzz.token_sort_ratio)
            match_names_fixture_salary_fuzzywuzzy(lst_A_names, lst_B_names, dict_names, scorer=fuzz.partial_token_sort_ratio)
